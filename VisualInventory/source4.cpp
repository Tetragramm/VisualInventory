...
int main() {
	...
	for (int i = 1; i < sortedPPts.size()-1; i++) {
		...
		if (sortedIPts[index].x > sortedIPts[i].x) {
			...
			Mat nImage;
			int x = 0;
			int y = sortedIPts[index].y - (sortedPPts[index].y - impose((int)sortedPPts[index].y));
			int width = sortedPPts[i].x - sortedPPts[index].x;
			getRectSubPix(image3, Size(width, height1), Point2f(x + width / 2, y + height1 / 2), nImage);
			subImages2.push_back(nImage);
			...
			subImages.push_back(pano(Rect(sortedPPts[index].x, impose((int)sortedPPts[index].y), sortedPPts[i].x - sortedPPts[index].x, height1)));
			/*
			 *Calculate the x, y, and width of the rectangle so that you can create the subImage.
			*/
			int x = sortedIPts[index].x;
			int y = sortedIPts[index].y - (sortedPPts[index].y - impose((int)sortedPPts[index].y));
			int width = sortedPPts[i].x - sortedPPts[index].x;
			Mat nImage;
			getRectSubPix(pano2, Size(width, height1), Point2f(x + (width / 2), y + (height1 / 2)), nImage);
			subImages2.push_back(nImage);
	...
	if (sortedIPts[index].x > sortedIPts[0].x) {
		...
		Mat image1 = pano2(Rect(sortedIPts[index].x, 0, pano2.cols - sortedIPts[index].x, pano2.rows));
		Mat image2 = pano2(Rect(0, 0, sortedIPts[0].x, pano2.rows));
		Mat image3;
		hconcat(image1, image2, image3);
		Mat nImage;
		int x = 0;
		int y = sortedIPts[index].y - (sortedPPts[index].y - impose((int)sortedPPts[index].y));
		int width = sortedPPts[0].x + (pano.cols-sortedPPts[index].x);
		getRectSubPix(image3, Size(width, height1), Point2f(x + width / 2, y + height1 / 2), nImage);
		subImages2.push_back(nImage);
		//Mat image1 = pano2(Rect(sortedIPts[index].x, impose((int)sortedIPts[index].y), pano2.cols - sortedIPts[index].x, height2));
	...
	for (int i = 0; i < subImages.size(); i++) {
		...
		Mat image1 = subImages[i];
		Mat image2 = subImages2[i];
		absdiff(image1, image2, image1);
		subImageDiffs.push_back(image1);
	}

	int newHeight = minHeight - (SECTION_HEIGHT -minY1);
	...
	imwrite("difference2.png", diffPano);
	waitKey();
